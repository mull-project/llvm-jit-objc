...
TEST(LLVMJIT, Test007_Category_Basic) {
    // These lines are needed for TargetMachine TM to be created correctly.
  llvm::InitializeNativeTarget();
  llvm::InitializeNativeTargetAsmPrinter();
  llvm::InitializeNativeTargetAsmParser();
  ...
  char fixturePath[255];
  snprintf(fixturePath, sizeof(fixturePath), "%s/%s", FixturesPath, "007_category_basic.bc");
  ...
  std::string cacheName("/tmp/Test007_Category_Basic.o");
  std::error_code EC;
